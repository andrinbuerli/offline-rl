...
help:: ##@Usage show this help
	@echo ""
	@printf "%30s \n\n" "${TURQUOISE}VARIABLES"
	@printf "%-15s %s \t %s \n" "" "${WHITE}DATA_PATH" "set the DATA_PATH variable to the local path of the datasets, default is the path '../data/'"
	@printf "%-15s %s \t\t %s \n" "" "FOLDER" "set the folder to use for the selected target ('')"
	@printf "%-15s %s \t\t %s \n" "" "PARAMS" "set additional parameters for selected target (train, default is '')"
	@printf "%-15s %s \t %s \n" "" "API_ENDPOINT" "Elasticsearch api endpoint to use (default is 'http://nasal611.csem.local:8088')"
	@echo "${RESET}"
	@echo ""
